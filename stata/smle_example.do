...
smle own pop , peeravg(peeravg) npeers(npeers) replace
/* Confirm that files have been created */
...
rcof `"noisily smle own pop, peeravg(peeravg) npeers(npeers)"' == 602
/* There should be at least 2 variables */
rcof "noisily smle own , peeravg(peeravg) npeers(npeers) replace" == 102
/* First variable (own choice) should be binary and should vary */
rcof "noisily smle peeravg pop, peeravg(peeravg) npeers(npeers) replace" == 450
smle zero pop , peeravg(peeravg) npeers(npeers) replace /* THIS CRASHES THE FORTRAN PROGRAM IF WE EXECUTE */
smle one pop , peeravg(peeravg) npeers(npeers) replace /* THIS CRASHES THE FORTRAN PROGRAM IF WE EXECUTE */
/* Peer average should vary over the range 0 to 1 */
rcof "noisily smle own pop , peeravg(pop) npeers(npeers) replace" == 459
smle own pop , peeravg(zero) npeers(npeers) replace /* THIS CRASHES THE FORTRAN PROGRAM IF WE EXECUTE */
/* Number of peers should be a strictly positive integer. It does not need to vary */
rcof "noisily smle own pop, peeravg(peeravg) npeers(zero) replace" == 459
smle own pop, peeravg(peeravg) npeers(oneandahalf) replace /* THIS SHOULD GET REJECTED BUT WE DON'T CHECK WHETHER IT'S AN INTEGER */
smle own pop, peeravg(peeravg) npeers(one) replace 
/* All other variables should vary and be linearly independent */
rcof "noisily smle own one, peeravg(peeravg) npeers(npeers) replace" == 459
rcof "noisily smle own pop pop, peeravg(peeravg) npeers(npeers) replace" == 459  
...
rcof "noisily smle own pop , peeravg(peeravg) npeers(npeers) replace nsim(0)" == 198
rcof "noisily smle own pop , peeravg(peeravg) npeers(npeers) replace nsim(2.5)" == 198
smle own pop , peeravg(peeravg) npeers(npeers) replace nsim(1) `execute'
estimates store nsim_1

/* Set restarts (integer, minimum 0) */
rcof "noisily smle own pop , peeravg(peeravg) npeers(npeers) replace restarts(-1)" == 198
rcof "noisily smle own pop , peeravg(peeravg) npeers(npeers) replace restarts(2.5)" == 198
...
rcof "noisily smle own pop, peeravg(peeravg) npeers(npeers) replace aggregate(one)" == 459  
rcof "noisily smle own pop, peeravg(peeravg) npeers(npeers) replace aggregate(pop)" == 459  
...
smle own pop , peeravg(peeravg) npeers(npeers) replace simulator_type("g - this will be interpreted as GHK since only the first letter matters")
smle own pop , peeravg(peeravg) npeers(npeers) replace simulator_type("GHK")
...
smle own pop , peeravg(peeravg) npeers(npeers) replace equilibrium("Bounds") 
rcof `"noisily smle own pop , peeravg(peeravg) npeers(npeers) replace equilibrium("Plot") execute"' == 198
smle own pop , peeravg(peeravg) npeers(npeers) replace equilibrium("Plot") 
rcof `"noisily smle own pop , peeravg(peeravg) npeers(npeers) replace equilibrium("Minimum") execute"' == 198
smle own pop , peeravg(peeravg) npeers(npeers) replace equilibrium("Minimum") 
...
smle own pop , peeravg(peeravg) npeers(npeers) replace rhotype("Fixed") rho(0.2)
...
/* These options are not yet supported with execution */
smle own pop , peeravg(peeravg) npeers(npeers) replace rhotype("Interval") /* RUNS BUT ONLY READS FIRST ROW */
