...
program define smle, eclass
	...
	syntax varlist [if] [in] , [ AGGregate(varlist) /*
		...
		strings: */ EQuilibrium(string) rhotype(string) SIMulator_type(string) ]
	** Add defaults 
	*** Optionally on 
	...
	** Find/create folder
	...
	di "folder = `folder'"
	** Get file names
	local parmfile "parmfile.txt"
	local datafile "datafile.txt"
	local resultfile "resultfile.txt"
	local logfile "logfile.txt"
	local ufile "ufile.txt"
	local bootfile "bootfile.txt"
	if (c(os) == "Windows") {
	...
	** Prepare data for export
	...
	gettoken peer_choice xvars : xvars
	capture assert inrange(`peer_choice',0,1)
	if _rc {
		di as error "Peer choice variable `peer_choice' must be between 0 and 1"
		error 459
	}
	gettoken num_peers xvars : xvars
	capture assert `num_peers' > 0
	if _rc {
		di as error "Number of peers `num_peers' must be > 0"
		error 459 
	}
	** Create data file
	outsheet `own_choice' `peer_choice' `num_peers' `aggregate' `xvars' using "`folder'/`datafile'" if `touse', nonames nolabel `replace'
	** Create parameter file
	...
	if "`execute'" == "execute" {
		...
		winexec `smle_exe' ".\smle_tmp\parmfile.txt"
		...
		di "DONE." _newline 
		preserve
		quietly infile loglik rhox rhoe gamma cons `aggregate' `xvars' using resultfile.txt, clear
		rename gamma `peer_choice'
		mkmat loglik `peer_choice' `aggregate' `xvars' cons rhox rhoe, matrix(b)
		restore
		scalar loglik = b[1,1]
		...
		ereturn local underreporting_correction "`underreporting_correction'"
		ereturn local rho_type "`rhotype'"
		ereturn local fix_gamma "`fixgamma'"
